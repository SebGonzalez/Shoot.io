    ...
    public static void main(String[] args) {
        ...
        while (!Display.isCloseRequested()) {
           glClear(GL_COLOR_BUFFER_BIT);  // Clear the 2D contents of the window.
			...
			glTexCoord2f(1, 0);
			glVertex2i(Display.getWidth()/2+RessourcesFactory.getImage(TypeImage.PERSONNAGE).getTextureWidth(), Display.getHeight()/2-25);
			glTexCoord2f(1, 1);
			glVertex2i(Display.getWidth()/2+RessourcesFactory.getImage(TypeImage.PERSONNAGE).getTextureWidth(), Display.getHeight()/2+RessourcesFactory.getImage(TypeImage.PERSONNAGE).getTextureHeight());
			glTexCoord2f(0, 1);
			glVertex2i(Display.getWidth()/2-25, Display.getHeight()/2+RessourcesFactory.getImage(TypeImage.PERSONNAGE).getTextureHeight());
			glEnd();
